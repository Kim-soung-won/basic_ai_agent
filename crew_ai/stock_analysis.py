...
import yfinance as yf # yfinance 를 활용해 주식 정보를 가져오는 Tool
from datetime import datetime, timedelta
import pandas as pd
from IPython.display import Image

# META의 주가 정보를 가져오는 함수
ticker = yf.Ticker("META")

# help(ticker) # 제공하는 함수 확인

ticker.history(period='5d', interval='1d') # 총 5일치의 주가 정보를 가져옴

# annual_financials = ticker.get_financials() # 재무제표
annual_financials = ticker.get_balance_sheet() # 대차대조표
# annual_financials = ticker.get_income_stmt() # 손익계산서
# quarter_financials = ticker.get_financials(freq="yearly") # 분기별 재무제표

annual_financials


# yfinance 라이브러리로 최근 종가 불러오는 Tool
@tool
def latest_stock_price(ticker):
    # 툴의 역할을 자연어로 아래와 같이 명시해줘야한다.
    """
